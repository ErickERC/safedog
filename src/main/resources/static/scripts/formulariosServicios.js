...
let jsonData;
const url = `http://54.86.9.85:8080/api/safedog/servicios`;
...
document.addEventListener("DOMContentLoaded", function () {
  function toggleDropdowns() {
    ...
    services.forEach((service) => {
      const yesRadio = document.querySelector(
        `input[name="${service}"][value="Si"]`
      );
      const noRadio = document.querySelector(
        `input[name="${service}"][value="No"]`
      );
      const dropdown = document.querySelector(
        `.div-${service} .dropdown select`
      );
  ...
  idServicio.addEventListener("submit", function (event) {
    ...
    const servicios = {
      paseo:
        idServicio.querySelector('input[name="paseo"]:checked')?.value || "No",
      paseoTarifa: getSelectValue("paseo"),
      hospedaje:
        idServicio.querySelector('input[name="hospedaje"]:checked')?.value ||
        "No",
      hospedajeTarifa: getSelectValue("hospedaje"),
      guarderia:
        idServicio.querySelector('input[name="guarderia"]:checked')?.value ||
        "No",
      guarderiaTarifa: getSelectValue("guarderia"),
      baño:
        idServicio.querySelector('input[name="baño"]:checked')?.value || "No",
    ...
    services.forEach((service) => {
      const selectedRadio = idServicio.querySelector(
    ...
    // Enviar por API
    fetch("http://54.86.9.85:8080/api/safedog/servicios", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
